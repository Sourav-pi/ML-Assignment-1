...
def _block_trend_features(x, prefix, names, n_blocks=10):
    ...
    (block_std). Complements the whole-window linear _slope: a window that
    dips then recovers has ~zero net slope but real block_std, which a
...
def make_features_for_window(record, feature_names_out):
    ...
    # block_trend/block_std use the raw (not detrended) signal -- both are
...
def cmd_feature_engineering(protocol, test_dir, model_path, output_path):
    ...
    if(DEBUG): total = time.time() - run_start
